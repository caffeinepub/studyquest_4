...
actor {
  ...
  let paymentRequests = Map.empty<Principal, PaymentRequest>();
  ...
  // Free access codes generated by admin
  let freeCodes = Map.empty<Text, Bool>();
  ...
  // PIN-based admin claim
  public shared ({ caller }) func claimAdminWithPin(username : Text, pin : Text) : async Bool {
    if (username == "SiMBa" and pin == "898900") {
  ...
  // Register instantly with a secret bypass code or admin-generated free code
  public shared ({ caller }) func registerWithAccessCode(username : Text, code : Text) : async () {
    if (userProfiles.containsKey(caller)) {
      Runtime.trap("User already exists");
    };
    let isValid = (code == "89789") or freeCodes.containsKey(code);
    if (not isValid) {
      Runtime.trap("Invalid access code");
    };
    // Consume free code if it was admin-generated
    if (code != "89789") {
  ...
  // Admin: generate a one-time free access code
  public shared ({ caller }) func adminGenerateFreeCode() : async Text {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can generate free codes");
    };
    let t = Time.now().toText();
    let code = "FREE" # Text.fromIter(t.chars().filter(func(c) { c >= '0' and c <= '9' })) # freeCodes.size().toText();
    freeCodes.add(code, true);
    code;
  };

  // Submit a manual payment request (user calls this after paying via PhonePe)
  public shared ({ caller }) func submitPaymentRequest(username : Text, inviteCode : Text) : async () {
    ...
    paymentRequests.add(caller, request);
  };

  // Admin: get all pending payment requests
  public query ({ caller }) func getPendingPayments() : async [PaymentRequest] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can view payment requests");
    };
    let pending = paymentRequests.values().toArray();
    pending;
  };

  // Admin: approve a payment request - registers user and marks as paid
  public shared ({ caller }) func approvePayment(userPrincipal : Principal) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can approve payments");
    };
    switch (paymentRequests.get(userPrincipal)) {
      case (null) { Runtime.trap("Payment request not found") };
      case (?request) {
        // Register the user if not already registered
        if (not userProfiles.containsKey(userPrincipal)) {
          // Try to register with invite code, or just create profile directly
          var inviterPrincipal : ?Principal = null;
          for ((principal, profile) in userProfiles.entries()) {
            if (profile.inviteCode == request.inviteCode) {
              inviterPrincipal := ?principal;
            };
          };
          let inviteCode = generateInviteCode(userPrincipal);
          let newUser : UserProfile = {
          ...
          userProfiles.add(userPrincipal, newUser);
          accessControlState.userRoles.add(userPrincipal, #user);
          switch (inviterPrincipal) {
            case (?inviter) {
              let inviterProfile = getUserProfileInternal(inviter);
              userProfiles.add(inviter, {
          ...
          // Already registered, just mark as paid
          let existingProfile = getUserProfileInternal(userPrincipal);
          userProfiles.add(userPrincipal, {
            username = existingProfile.username;
            rank = existingProfile.rank;
            inviteCode = existingProfile.inviteCode;
            invites = existingProfile.invites;
            quizWins = existingProfile.quizWins;
            status = existingProfile.status;
            hasPaid = true;
          });
        };
        // Update request status to approved
        paymentRequests.add(userPrincipal, {
  ...
  public shared ({ caller }) func rejectPayment(userPrincipal : Principal) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can reject payments");
    };
    paymentRequests.remove(userPrincipal);
  };
